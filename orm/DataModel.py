...
class DataModel:
    ...
    Provides standardized implementation for reading and writing data from and to SQLAlchemy mapped tables.
    """
    ...
    class Prop:
        ...
        def __init__(self, attr, alias=None, flags=None, args=(), kwargs={}, mask=None, target=None, dispname=None,
            ...
            link : 2-tuple of str, optional
                Name of the attributes linking the bidirectional relationship. The default is None.
            filter : str, optional
            ...
            **unknown : dict
            ...
            if len(unknown):
                from api import app
                app.logger.warn("Unknown DataModel parameters: "+", ".join(unknown.keys()))
    ...
    def automatch(cls, query, expr, fields=None):
        ...
        filters = [and_(column.ilike(match) for match in matchexpr) for prop, column in targets if prop.match == "default"] +\
...
def Comment():
    ...
    return DataModel.Prop("comment", flags="ref, patch, managed", flat="comment")
...
def Name(attr="name", **kwargs):
    """Create a name property."""
    _addFlags(kwargs, "patch,sort,match")
    return DataModel.Prop(attr, filter="range", **kwargs)
...
def DateTime(attr, **kwargs):
    """Create a date attribute."""
    _addFlags(kwargs, "sort")
    return DataModel.Prop(attr,
                          func=lambda date: date.strftime("%Y-%m-%d %H:%M") if date else None,
                          filter="range", **kwargs)
